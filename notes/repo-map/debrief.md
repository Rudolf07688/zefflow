...
- **R1. Always run `git status` + `git log -1 --stat` first.** Treat any IDE-supplied tree or summary as potentially stale.
...
- **R5. Always do a deps↔imports cross-check.** Cheap, surfaces drift in seconds.
...
phase 2  Drill          │
  ...
  ├── read each pkg spine (config, base, registry, one concrete impl)
  ├── deps↔imports cross-check
  └── classify maturity ─┤
                         │
phase 3  Synthesize      │
